...
class General_Setup {
		...
		<?php if ( isset($_POST['price_search']) && $_POST['price_search'] == 'yes' ): ?>
            <div class="search-item">
                <div class="price-range">
                    <label><?php esc_html_e( 'Price', 'mytheme' ); ?></label>
					<?php
					$data_form = '';
					$data_to   = '';
	...
	function rt_show_extra_profile_fields( $user ) {
        ...
        <h3><?php esc_html_e( 'Agency Info', 'mytheme' ); ?></h3>

        <table class="form-table">
            <tr>
                <th><label for="user_agency"><?php esc_html_e( 'Select an Agency', 'mytheme' ); ?></label></th>
                <td>
					<?php
					$args       = [
						'post_type'      => 'store',
                    ...
                    <select name="user_agency" id="user_agency">
                        <option><?php echo esc_html__( 'Select Agency', 'mytheme' ) ?></option>
						<?php
	...
	public function register_sidebars() {
		...
		register_sidebar(
			[
				'name'          => esc_html__( 'Single Property', 'mytheme' ),
		...
		register_sidebar(
			[
				'name'          => esc_html__( 'Agency/Store Sidebar', 'mytheme' ),
				'id'            => 'store-sidebar',
				'before_widget' => '<div id="%1$s" class="widget %2$s">',
				'after_widget'  => '</div>',
				'before_title'  => '<h3 class="widget-heading">',
		...
		register_sidebar(
			[
				'name'          => esc_html__( 'Agent Sidebar', 'mytheme' ),
				'id'            => 'agent-sidebar',
				'before_widget' => '<div id="%1$s" class="widget %2$s">',
				'after_widget'  => '</div>',
				'before_title'  => '<h3 class="widget-heading">',
		...
		register_sidebar(
			[
				'name'          => esc_html__( 'Listing Archive Sidebar', 'mytheme' ),
		...
		if ( class_exists( 'WooCommerce' ) ) {
			register_sidebar(
				[
					'name'          => esc_html__( 'WooCommerce Archive Sidebar', 'mytheme' ),
			...
			register_sidebar(
				[
					'name'          => esc_html__( 'WooCommerce Single Sidebar', 'mytheme' ),
	...
	public function body_classes( $classes ) {
		...
		$mytheme_theme = wp_get_theme();
		$classes[]      = $mytheme_theme->Name . '-version-' . $mytheme_theme->Version;
	...
	public function breadcrumb() {
		...
		if ( ! empty( $args['breadcrumb'] ) ) {
            ...
            <section class="breadcrumbs-banner <?php echo esc_attr( $breadcrumb_style ) ?>">
                <div class="container">
					<?php if ( MyTheme::$options['breadcrumb_title'] !== 'disable' ) :

						//oldCondition: is_single() ? 'screen-reader-text' :
						?>
                        <h1 class="<?php echo esc_attr( MyTheme::$options['breadcrumb_title'] ); ?>">
							<?php
	...
	public function homlist_app_notice_dismiss() {
		if ( check_ajax_referer( "dismissnotice", "nonce" ) ) {
			if ( isset( $_POST['dismiss'] ) && $_POST['dismiss'] == 1 ) {
				set_transient( "mytheme_dismiss_notice", 1, MONTH_IN_SECONDS * 2 );
				wp_die( "done" );
			}
		}
	}
