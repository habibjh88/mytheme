...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			// Load class strings.
			$this->strings = array(
				'page_title'                      => esc_html__( 'Install Required Plugins', 'mytheme' ),
				'menu_title'                      => esc_html__( 'Install Plugins', 'mytheme' ),
				/* translators: %s: plugin name. */
				'installing'                      => esc_html__( 'Installing Plugin: %s', 'mytheme' ),
				/* translators: %s: plugin name. */
				'updating'                        => esc_html__( 'Updating Plugin: %s', 'mytheme' ),
				'oops'                            => esc_html__( 'Something went wrong with the plugin API.', 'mytheme' ),
				...
				'return'                          => esc_html__( 'Return to Required Plugins Installer', 'mytheme' ),
				'dashboard'                       => esc_html__( 'Return to the Dashboard', 'mytheme' ),
				'plugin_activated'                => esc_html__( 'Plugin activated successfully.', 'mytheme' ),
				'activated_successfully'          => esc_html__( 'The following plugin was activated successfully:', 'mytheme' ),
				/* translators: 1: plugin name. */
				'plugin_already_active'           => esc_html__( 'No action taken. Plugin %1$s was already active.', 'mytheme' ),
				/* translators: 1: plugin name. */
				'plugin_needs_higher_version'     => esc_html__( 'Plugin not activated. A higher version of %s is needed for this theme. Please update the plugin.', 'mytheme' ),
				/* translators: 1: dashboard link. */
				'complete'                        => esc_html__( 'All plugins installed and activated successfully. %1$s', 'mytheme' ),
				'dismiss'                         => esc_html__( 'Dismiss this notice', 'mytheme' ),
				'notice_cannot_install_activate'  => esc_html__( 'There are one or more required or recommended plugins to install, update or activate.', 'mytheme' ),
				'contact_admin'                   => esc_html__( 'Please contact the administrator of this site for help.', 'mytheme' ),
			);
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_plugin_advise_type_text( $required ) {
			if ( true === $required ) {
				return esc_html__( 'Required', 'mytheme' );
			}

			return esc_html__( 'Recommended', 'mytheme' );
		}
		...
		protected function get_plugin_source_type_text( $type ) {
			...
			switch ( $type ) {
				case 'repo':
					$string = esc_html__( 'WordPress Repository', 'mytheme' );
					break;
				case 'external':
					$string = esc_html__( 'External Source', 'mytheme' );
					break;
				case 'bundled':
					$string = '<span></span>'.esc_html__( 'Pre-Packaged', 'mytheme' );
					break;
			}
		...
		protected function get_plugin_status_text( $slug ) {
			if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
				return esc_html__( 'Not Installed', 'mytheme' );
			}

			if ( ! $this->tgmpa->is_plugin_active( $slug ) ) {
				$install_status = esc_html__( 'Installed But Not Activated', 'mytheme' );
			} else {
				$install_status = esc_html__( 'Active', 'mytheme' );
			}

			$update_status = '';

			if ( $this->tgmpa->does_plugin_require_update( $slug ) && false === $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = esc_html__( 'Required Update not Available', 'mytheme' );

			} elseif ( $this->tgmpa->does_plugin_require_update( $slug ) ) {
				$update_status = esc_html__( 'Requires Update', 'mytheme' );

			} elseif ( false !== $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = esc_html__( 'Update recommended', 'mytheme' );
			}
		...
		public function column_version( $item ) {
			...
			if ( $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				...
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . esc_html__( 'Installed version:', 'mytheme' ) . '</p>',
					$color,
					$installed
				);
			}
			...
			if ( ! empty( $item['minimum_version'] ) ) {
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;">%1$s</span>' . esc_html__( 'Minimum required version:', 'mytheme' ) . '</p>',
			...
			if ( ! empty( $item['available_version'] ) ) {
				...
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . esc_html__( 'Available version:', 'mytheme' ) . '</p>',
					$color,
		...
		public function get_columns() {
			$columns = array(
				...
				'plugin' => esc_html__( 'Plugin', 'mytheme' ),
				'source' => esc_html__( 'Source', 'mytheme' ),
				'type'   => esc_html__( 'Type', 'mytheme' ),
			);

			if ( 'all' === $this->view_context || 'update' === $this->view_context ) {
				$columns['version'] = esc_html__( 'Version', 'mytheme' );
				$columns['status']  = esc_html__( 'Status', 'mytheme' );
			}
		...
		protected function get_row_actions( $item ) {
			...
			if ( ! $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				...
				$actions['install'] = esc_html__( 'Install %2$s', 'mytheme' );
			} else {
				// Display the 'Update' action link if an update is available and WP complies with plugin minimum.
				if ( false !== $this->tgmpa->does_plugin_have_update( $item['slug'] ) && $this->tgmpa->can_plugin_update( $item['slug'] ) ) {
					/* translators: %2$s: plugin name in screen reader markup */
					$actions['update'] = esc_html__( 'Update %2$s', 'mytheme' );
				}

				// Display the 'Activate' action link, but only if the plugin meets the minimum version.
				if ( $this->tgmpa->can_plugin_activate( $item['slug'] ) ) {
					/* translators: %2$s: plugin name in screen reader markup */
					$actions['activate'] = esc_html__( 'Activate %2$s', 'mytheme' );
				}
			}
		...
		public function get_bulk_actions() {
			...
			if ( 'update' !== $this->view_context && 'activate' !== $this->view_context ) {
				if ( current_user_can( 'install_plugins' ) ) {
					$actions['tgmpa-bulk-install'] = esc_html__( 'Install', 'mytheme' );
				}
			}

			if ( 'install' !== $this->view_context ) {
				if ( current_user_can( 'update_plugins' ) ) {
					$actions['tgmpa-bulk-update'] = esc_html__( 'Update', 'mytheme' );
				}
				if ( current_user_can( 'activate_plugins' ) ) {
					$actions['tgmpa-bulk-activate'] = esc_html__( 'Activate', 'mytheme' );
				}
			}
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
				...
				if ( empty( $_POST['plugin'] ) ) {
					if ( 'install' === $install_type ) {
						$message = esc_html__( 'No plugins were selected to be installed. No action taken.', 'mytheme' );
					} else {
						$message = esc_html__( 'No plugins were selected to be updated. No action taken.', 'mytheme' );
					}
				...
				if ( empty( $plugins_to_install ) ) {
					if ( 'install' === $install_type ) {
						$message = esc_html__( 'No plugins are available to be installed at this time.', 'mytheme' );
					} else {
						$message = esc_html__( 'No plugins are available to be updated at this time.', 'mytheme' );
					}
			...
			if ( ! class_exists( 'TGMPA_Bulk_Installer' ) ) {
				...
				class TGMPA_Bulk_Installer extends Plugin_Upgrader {
					...
					public function activate_strings() {
						$this->strings['activation_failed']  = esc_html__( 'Plugin activation failed.', 'mytheme' );
						$this->strings['activation_success'] = esc_html__( 'Plugin activated successfully.', 'mytheme' );
					}
					...
					public function add_strings() {
						if ( 'update' === $this->options['install_type'] ) {
							...
							if ( $this->tgmpa->is_automatic ) {
								...
								/* translators: 1: plugin name. */
								$this->upgrader->strings['skin_update_successful'] = esc_html__( '%1$s installed and activated successfully.', 'mytheme' ); // @kowsar
								...
								/* translators: 1: plugin name. */
								$this->upgrader->strings['skin_update_successful'] = esc_html__( '%1$s installed successfully.', 'mytheme' ); // @kowsar
